 ...
 *        in the log entry
 */
Glass.prototype.logDebug = function(message, object, logTime) {
	var self = this;
	if(typeof logTime === 'undefined')
		logTime = true;
	self.log(self.LOG_DEBUG, logTime, message, object);
}
 ...
 *        in the log entry
 */
Glass.prototype.logInfo = function(message, object, logTime) {
	var self = this;
	if(typeof logTime === 'undefined')
		logTime = true;
	self.log(self.LOG_INFO, logTime, message, object);
}
 ...
 *        in the log entry
 */
Glass.prototype.logOK = function(message, object, logTime) {
	var self = this;
	if(typeof logTime === 'undefined')
		logTime = true;
	self.log(self.LOG_OK, logTime, message, object);
}
 ...
 *        in the log entry
 */
Glass.prototype.logWarning = function(message, object, logTime) {
	var self = this;
	if(typeof logTime === 'undefined')
		logTime = true;
	self.log(self.LOG_WARNING, logTime, message, object);
}
 ...
 *        in the log entry
 */
Glass.prototype.logError = function(message, object, logTime) {
	var self = this;
	if(typeof logTime === 'undefined')
		logTime = true;
	self.log(self.LOG_ERROR, logTime, message, object);
}
 ...
 *        displayed under the message text
 */
Glass.prototype.log = function(severity, logTime, message, object) {
	var self = this;
	var ts = '';
	var w;
	...
	switch(severity) {
		case self.LOG_DEBUG:
			if(self.logMode == 1) {
				w = '[DBG' + ts + '] ';
				console.log(w.grey + message);
			}
			break;
		case self.LOG_OK:
			w = '[OK ' + ts + '] ';
			console.log(w.green + message);
			break;
		case self.LOG_WARNING:
			w = '[WRN' + ts + '] ';
			console.log(w.yellow + message);
			break;
		case self.LOG_ERROR:
			w = '[ERR' + ts + '] ';
			console.log(w.red + message);
			break;
		default:
			w = '[INF' + ts + '] ';
			console.log(w.white + message);
			break;
	}
	...
	if(!attachment || attachment == '-' || !fs.existsSync(attachment)) {
		if(attachment && attachment != '-')
			...
			if(lat && lon) {
				...
				self.getMap(lat, lon, zoom, marker,
					errorCallback,
...
Glass.prototype.getMapUrl = function(lat, lon, zoom, marker) {
	var position;
	if(marker && marker != '-')
...
Glass.prototype.getMap = function(lat, lon, zoom, marker, 
		errorCallback, successCallback) {
	var self = this;
	var mapUrl = self.getMapUrl(lat, lon, zoom, marker);
